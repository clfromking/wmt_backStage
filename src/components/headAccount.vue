	...
	<div>
		<div class="header">
			<el-breadcrumb separator-class="el-icon-arrow-right header-title">
				<el-breadcrumb-item :to="{path:'/storeBiddingAccount'}">总部账户</el-breadcrumb-item>
				<!-- <el-breadcrumb-item>账户详情</el-breadcrumb-item> -->
			</el-breadcrumb>
			<div class="allPrice">
				<span>总余额：</span>
				<span>{{totalPrice}}</span>
				<el-button type="primary" @click='goHeadAccountPay(0)'>充值</el-button>
				<el-button type="success" @click='goHeadAccountPay(1)'>提现</el-button>
...
// 				return
			...
			//提现或充值
			goHeadAccountPay:function(type){
				// console.log(type)
				// return
				var query = {
					type:type,
...
// 					poiId:this.$route.query.id,
			...
			loadTableData: function() {
				...
				else if (this.postOption2 == '提现') {
					postData.tradingType = 1
				} 
				else if (this.postOption2 == '商户余额充值') {
					postData.tradingType = 2
				} 
				else if (this.postOption2 == '商户竞价充值') {
					postData.tradingType = 3
				} 
				else if (this.postOption2 == '商户订单支付') {
					postData.tradingType = 4
				} 
				else if (this.postOption2 == '充值') {
					postData.tradingType = 5
				} 
...
// 				return
				this.axios.post('/mgr/account/tran/list', postData).then(res => {
					console.log(res)
					if (res.data.code == 200) {
						...
						for (let i = 0; i < list.length; i++) {
							...
							switch(Number(list[i].tradingType)){
								case 1:
									list[i].tranTypeText = '提现'
									break;
								case 2:
									list[i].tranTypeText = '商户余额充值'
									break;
								case 3:
									list[i].tranTypeText = '商户竞价充值'
									break;
								case 4:
									list[i].tranTypeText = '商户订单支付'
									break;
								case 5:
									list[i].tranTypeText = '充值'
									break;
								default:
									list[i].tranTypeText = '未知'
									break;
							}
						...
						this.totalPrice = '¥' + (Number(res.data.data.sumAmount)/100).toFixed(2)
