<template>
	<div class="body">
		<el-breadcrumb separator-class="el-icon-arrow-right header-title">
			<el-breadcrumb-item :to="{ path: '/takeOutOperating' }">外卖运营</el-breadcrumb-item>
	...
	export default{
		...
		methods: {
			...
			loadDetail:function(){
				...
				this.axios.post('/mgr/poi/request/detail',{"accessToken":localStorage.accessToken,'id':this.$route.query.id,'usefor':1}).then(res=>{
					console.log(res)
					if(res.data.code == 200){
						...
						this.$alert.error('服务单号不存在，请重新核对')
						setTimeout(()=>{
							this.$router.back(-1)
						},1500)
					}
